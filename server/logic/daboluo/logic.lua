...
function logic.login(user)
    ...
    LOG("some one login===============(roomid,userid,seatid):", g.room_info.roomid, user.id, seatid)
    if user.seat == nil and (not user.is_robot) and g.robot_user > 0 then    --座位已满,但是存在机器人,可替换  
        --user.seat = g.game_logic.remove_one_robot() -- 移除机器人
        --logic.someone_sit(user.seat, user)
        if g.new_gamer[1] == nil then
            g.new_gamer[1] = user
        else
            g.new_gamer[2] = user
        end
        print("g.new_gamer=====================", g.new_gamer)
    end
...
function logic.post_leave(user)
    print("logic.post_leave=================================user.id, g.game_step", user.id, g.game_step)
    
    -- 如果游戏中，则不清空用户数据
    if g.game_step ~= Config_daboluo.step_none and user.seat then
        user.offline = true
    else
        user.id = tonumber(user.id)
        g.users[user.id] = nil   --清空agent用户数据
    ...
    if not user.is_robot then
        ...
        user.id = tostring(user.id)   --由于exit_member时大厅的g.user[id]的id值为string型,所以在此进行类型转换
...
function logic.reconn(fd, id)
    ...
    local res = {
        ...
        resumeTime = g.client_time or Config_daboluo.auto_card_time-g.game_time,  -- 剩余时间
    ...
    if g.game_step == Config_daboluo.step_compare and g.scores then
        res.swat = g.scores.all_swat
        res.shootLen = g.scores.shootLen
    elseif g.game_step == Config_daboluo.step_none or g.game_step == Config_daboluo.step_over then-- 上一把的牌数据
    ...
    if g.cards then
        for k,v in pairs(g.cards) do
            ...
            if g.game_step == Config_daboluo.step_compare then
...
function logic.room_deal_card()
    local max_card_num = g.room_info.card_num   --17
    local room_type = g.room_type   --9

    g.cards = {}
    local cards = logic.shuffle_card(max_card_num, g.room_info.max_count)
    print("room_deal_card:room_type,variety===========", room_type, g.room_info.variety)
    local cards_str = {}
    print("g.seat===========>", g.seat)
    for k,v in pairs(g.seat) do
...
function logic.someone_sit( seat, user )
    if seat then
        ...
        -- 如果游戏还没开始，则步骤时间清零
        if g.game_step == Config_daboluo.step_none then
            g.game_time = 0
        end
    end
end
...
function logic.remove_one_robot()
    for k,v in pairs(g.users) do
        if v.is_robot then
            v.offline = true
            
            print("remove_one_robot=====================(userid, seatid, is_robot):", k,  v.seat, v.is_robot)

            --logic.someone_stand(v)
...
function logic.try_quit(user)
    ...
    print("try_quit=================== ", g.room_info.private, (g.game_step == Config_daboluo.step_deal_card))
    if not g.room_info.private then
        if (g.game_step == Config_daboluo.step_deal_card) and user.seat then -- 游戏中
            return true
        end
    else
