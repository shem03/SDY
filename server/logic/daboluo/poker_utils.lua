...
--四条（铁支）
function PokerUtils.sitiao(cards, varietyC)
	if #cards < 5 then return {} end
	local cArr = PokerUtils.getGroupCards(cards, varietyC)
...
PokerUtils.GroupName = {
	"乌龙","对子","两对","三条","顺子","同花","葫芦","四条", "同花顺", "五同"
}
...
--牌型附加分
-- [乌龙1，对子2，两对3，三条4，顺子5，同花6，葫芦7，金刚8，同花顺9, 五同10 中墩葫芦13]+皇家同花顺
function PokerUtils.getNomalScore(pos, com, danse, cards)
	local value = nil
	local function get_top_value( v1, v2, v3 )  --计算头道对子或三条的牌值
		if (v1 == v2)  or (v1 == v3) then
			return v1
		else 
			return v2
		end
	...
	--底道
	if pos == 1 then   
		if com == 5  then  value = 2 end   --顺子
		if com == 6  then  value = 4 end   --同花
		if com == 7  then  value = 6 end   --葫芦
		if com == 8  then  value = 10 end   --金刚
		if com == 9  then  value = 15 end   --同花顺
		if com == 99  then  value = 25 end   --皇家同花顺
	end

	--中道
	if pos == 2 then
		if (com == 4 or com == 11 )  then  value = 2 end   --三条
		if com == 5  then  value = 4 end   --顺子
		if com == 6  then  value = 8 end   --同花
		if com == 7  then  value = 12 end   --葫芦
		if com == 8  then  value = 20 end   --金刚
		if com == 9  then  value = 30 end   --同花顺
		if com == 99  then  value = 50 end   --皇家同花顺
	end

	--头道
	if pos == 3 then
		local top_value = get_top_value( cards[11].value, cards[12].value, cards[13].value )
		if com == 2 then   --对子
			if top_value == 6  then  value = 2 end   --77
			if top_value == 7  then  value = 3 end   --88
			if top_value == 8  then  value = 4 end   --99
			if top_value == 9  then  value = 5 end   --10,10
			if top_value == 10  then  value = 6 end   --JJ
			if top_value == 11  then  value = 7 end   --QQ
			if top_value == 12  then  value = 8 end   --KK
			if top_value == 13  then  value = 9 end   --AA
		end
		if com == 4 then  --三条
			if top_value == 1  then  value = 10 end   --222
			if top_value == 2  then  value = 11 end   --333
			if top_value == 3  then  value = 12 end   --444
			if top_value == 4  then  value = 13 end   --555
			if top_value == 5  then  value = 14 end   --666
			if top_value == 6  then  value = 15 end   --777
			if top_value == 7  then  value = 16 end   --888
			if top_value == 8  then  value = 17 end   --999
			if top_value == 9  then  value = 18 end   --10,10,10
			if top_value == 10  then  value = 19 end   --JJ
			if top_value == 11  then  value = 20 end   --QQ
			if top_value == 12  then  value = 21 end   --KK
			if top_value == 13  then  value = 22 end   --AA
		end
	end
	
	--print("getNomalScore=======================pos,com,value", pos, com, value)
	return value
end
...
function PokerUtils.compare_sitiao( index, a, b )
...
--同牌型对比func
PokerUtils.compare = {
	[1] = PokerUtils.compare_wulong,   --乌龙
	[2] = PokerUtils.compare_yidui,    --一对
	[3] = PokerUtils.compare_liangdui, --两对
	[4] = PokerUtils.compare_santiao,  --三条
	[5] = PokerUtils.compare_shunzi,   --顺子
	[6] = PokerUtils.compare_tonghua,  --同花
	[7] = PokerUtils.compare_hulu,     --葫芦
	[8] = PokerUtils.compare_sitiao,   --四条（金刚）
	[9] = PokerUtils.compare_tonghuashun,  --同花顺
	[10] = PokerUtils.compare_wutong,	--五同
}
