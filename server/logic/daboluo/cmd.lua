...
function cmd.try_stand( user, data )
    ...
    if g.game_step > Config_daboluo.step_none and g.game_step ~= Config_daboluo.step_over and g.cards[user.id] then
...
function cmd.change_room( user, data )
     if g.game_step > Config_daboluo.step_none and user.seat then -- 游戏中
...
function cmd.choice_poker_type( user, data )
    ...
    if not g.room_info.variety then
        local old_cards = user_cards.user_card   --全部手牌
        local res_cards = poker_utils.delCards(cards, old_cards)  --剔除13张提交牌，剩下的牌
        if data.left_card then    --如果还有剩余的手牌
