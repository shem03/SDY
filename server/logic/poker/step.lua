...
local cheat = require("poker.cheat")
local test_card = require("poker.test_card")
local cmd = {}
    ...
    if g.room_info.room_coin > 0 then
        for i,v in ipairs(g.seat) do
            ...
            if not user.is_robot then
                -- 收取金币10作为手续费
                if room_logic.user_update_coin(user, -Config.table_fee) == false then
                    assert(false, "手续费金币不足:"..user.id)
                end

                send_msg(user.fd, {
                    msg_id="update_bee", 
                    cur_coin=user.user_coin
                })
            end
        end
    end
...
function cmd.run()
    if Config.robot and g.room_info.private == false then -- 如果私人场，则不添加机器人
        if g.real_user + g.robot_user < g.room_info.min_count and g.game_logic.add_robot and g.game_step == Config.step_none then
            g.game_logic.add_robot()
        end
    end

	g.game_time = g.game_time + 1
    -- print("cmd.run:", g.game_time, g.game_step, g.seat)
...
-- 开局
cmd[Config.step_fee] = function()
    if g.game_time < 2 then
        return
    end
	g.game_step = Config.step_deal_card
    g.cur_cards = {}
    g.game_time = 0
    local data = {
        msg_id = "round_start",
...
-- 发牌
cmd[Config.step_deal_card] = function ()
    ...
    for k,v in pairs(g.cards) do
        ...
        --if not g.room_info.private then -- 私人房不自动出牌  -- 大赢家私人自动出牌
            ...
            if g.cur_cards[k] == nil and g.game_time > Config.auto_card_time then
    ...
    if has_nil == false then
	    -- 如果全部出牌完毕，则进行比牌
	    g.game_step = Config.step_compare
	    g.game_time = 0
	end
end
...
cmd[Config.step_compare] = function()
    if g.client_time == nil then
        ...
        if cur_cards.left_card then
            ...
            table.insert(log_compare, poker_utils.get_cardlog(cur_cards.left_card))
        end
        -- 计算分数
