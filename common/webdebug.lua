...
local function dump_list(print, list, isweb)
	local index = {}
...
local function docmd(cmdline, print, fd, isweb)
	LOG("webdebug docmd:", cmdline, isweb)
...
local function console_main_loop(stdin, print)
	print('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>控制台</title></head><body><p>')
	print("Welcome to web console<br><br>")
	...
	pcall(function()
		while true do
			...
			if cmdline:sub(1,4) == "GET " then
				...
				docmd(cmdline, print, stdin, true)
				break
			end
			if cmdline ~= "" then
				docmd(cmdline, print, stdin, true)
...
skynet.start(function()
	...
	skynet.error("Start debug console at 0.0.0.0 " .. port)
...
function COMMAND.call( fd, address, cmd )
	print('call:', address, cmd)
	local res = skynet.call(address, "lua", cmd)
	return cmd..':'..res
end
